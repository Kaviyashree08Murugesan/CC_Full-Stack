...
// Fetch Profile Data via Java Backend API (or direct GitHub API fallback)
async function fetchGitHubProfile(username) {
    ...
    try {
        ...
        // Attempt fetching from Java Backend REST API first
        try {
            const apiResponse = await fetch(`/api/github/user/${username}`);
            if (apiResponse.ok) {
                const data = await apiResponse.json();
                user = data.profile;
                repos = data.repositories || [];
            } else if (apiResponse.status === 404) {
                showError('User Not Found', `The requested GitHub username '${username}' does not exist.`);
                return;
            }
        } catch (backendErr) {
            console.info('Backend API unavailable, attempting direct GitHub REST API call...', backendErr);
        }

        // Direct GitHub REST API fallback if backend proxy not used
        if (!user) {
...
function displayProfile(user, repos) {
    ...
    // Stats
    publicRepos.textContent = (user.public_repos || user.publicRepos || 0).toLocaleString();
    followers.textContent = (user.followers || 0).toLocaleString();
    following.textContent = (user.following || 0).toLocaleString();
    ...
    // Twitter
    const twitterHandle = user.twitter_username || user.twitterUsername;
    if (twitterHandle) {
        twitterText.textContent = `@${twitterHandle}`;
...
function renderRepositories(repos) {
    ...
    repos.forEach(repo => {
        ...
        repoCard.href = repo.html_url || repo.htmlUrl;
        repoCard.target = '_blank';
        repoCard.rel = 'noopener noreferrer';

        const repoLanguage = repo.language || 'Plain Text';
        const stars = repo.stargazers_count !== undefined ? repo.stargazers_count : (repo.stargazersCount || 0);
